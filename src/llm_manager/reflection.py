...
class ReflectionPromptBuilder:
    ...
    def build_prompt(self, original_query: str, previous_response: str) -> str:
    ...
    def _build_self_critique_prompt(self, original_query: str, previous_response: str) -> str:
    ...
    def _build_verification_prompt(self, original_query: str, previous_response: str) -> str:
        """Builds a verification prompt."""
        return verification_prompt_template.format(query=original_query, response=previous_response)

    def _build_adversarial_prompt(self, original_query: str, previous_response: str) -> str:
        """Builds an adversarial prompt."""
        return adversarial_prompt_template.format(query=original_query, response=previous_response)
...
class ReflectiveLLMManager:
    ...
    def reflect(
        self, user_query: str, reflection_strategy: str, num_iterations: int, **kwargs
    ) -> ReflectionResult:
        ...
        for iteration_num in range(num_iterations):
            ...
            reflection_prompt = prompt_builder.build_prompt(
                original_query=user_query, previous_response=previous_response.text
            )
